...
def simple(g):
    ...
    # YOUR CODE HERE
    sec = g.edges.select("dst", "src")
    edg = sec.unionAll(g.edges)
        
    g = GraphFrame(g.vertices,edg)
    #g.edges.show()
    ...
    # YOUR CODE HERE
    c = g.edges.filter("src!=dst")
    
    return GraphFrame(g.vertices,c)
...
def degreedist(g):
    ...
    # YOUR CODE HERE
    count = g.inDegrees.selectExpr('id as id','inDegree as degree')\
            .groupBy('degree').count()
        
    #print(type(count))
    nodes = [int(row['count']) for row in count.collect()]
    degrees = [int(row['degree']) for row in count.collect()]
    pyplt.plot(degrees, nodes)
    pyplt.ylabel('Count')
    pyplt.xlabel('Degree')
    pyplt.savefig(out+".png")
    print(powerlaw.Fit(nodes).power_law.alpha)
    return count
...
def readFile(filename, large, sqlContext=sqlContext):
    ...
    # YOUR CODE HERE
    p = lines.map(lambda l: l.split(delim))
    ...
    # graphframe vertices
    # YOUR CODE HERE
	edges = sqlContext.createDataFrame(p,['src','dst'])
